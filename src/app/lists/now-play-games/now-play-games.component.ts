...
`,
styleUrls: ['./now-play-games.component.css'],
imports: [
  CommonModule, 
  HttpClientModule, 
  GameCardComponent, 
...
export class NowPlayGamesComponent implements OnInit {
  games: Game[] = [];
  filteredGames: Game[] = [];
  ...
  async ngOnInit() {
    ...
    try {
      this.games = await this.game_service.getGamesForList('Currently Playing');  // Await the Promise to get the games
